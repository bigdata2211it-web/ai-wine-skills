...
description: >
  ...
  .exe.config, missing DirectX/D3DX/D3DCompiler runtime, missing fonts
  (corefonts/cjkfonts, host fontconfig integration), missing Visual
  C++ runtimes (MSVCRT/MSVCR/MSVCP family mapping to vcrun* verbs),
  MSI/InstallShield/NSIS installers, COM/ActiveX registration via
  regsvr32, registry probes via WINEDEBUG=reg, the winetricks
  remove_mono hang workaround, DXVK/Vulkan acceleration, and creating
  ...
  Direct3D device", "missing MSVCR140.dll", "tofu boxes wine",
  "сделай ярлык для wine приложения", or when an agent needs to triage
  a Wine launch failure for a .NET/WinForms or native Win32 application.
---
