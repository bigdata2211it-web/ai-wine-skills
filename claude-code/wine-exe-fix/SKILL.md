...
name: wine-exe-fix
description: |
  Diagnose and fix Windows .exe launch failures under Wine on Linux. Covers
  32-bit vs 64-bit Wine prefix selection, missing .NET runtime (Wine Mono / MS
  .NET via winetricks), missing NuGet-managed dependencies, the distinction
  between NuGet package version and .NET assembly version, target-framework
  variants (lib/net40 vs lib/net45) inside one .nupkg, FileLoadException
  "manifest does not match" and binding redirects in .exe.config, missing
  DirectX/D3DX/D3DCompiler runtime, missing fonts (corefonts/cjkfonts + host
  fontconfig), missing Visual C++ runtimes (MSVCRT/MSVCR/MSVCP mapping to
  vcrun* verbs), MSI/InstallShield/NSIS installers, COM/ActiveX registration
  via regsvr32, registry probes via WINEDEBUG=reg, winetricks remove_mono
  hang workaround, DXVK/Vulkan acceleration, and creating reusable .desktop
  launchers that pin a custom WINEPREFIX. Use when the user says "exe не
  запускается под wine", "wine ошибка 32/64", "Could not load assembly",
  "Wine Mono is not installed", "FileLoadException manifest does not match",
  "Failed to create Direct3D device", "missing MSVCR140.dll", "tofu boxes
  wine", "сделай ярлык для wine приложения", or when an agent needs to
  triage a Wine launch failure for a .NET/WinForms or native Win32
  application.
---
