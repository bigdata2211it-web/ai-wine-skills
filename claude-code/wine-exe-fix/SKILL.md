...
description: |
  ...
  "manifest does not match" and binding redirects in .exe.config, the
  winetricks remove_mono hang workaround, and creating reusable .desktop
  ...
  "сделай ярлык для wine приложения", or when an agent needs to triage a
  Wine launch failure for a .NET/WinForms application.
---
