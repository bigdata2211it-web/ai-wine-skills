...
> installed", "FileLoadException manifest does not match", "сделай ярлык для
> wine приложения". Apply when the user wants to run a Windows .exe under
> Wine and hits an error, or asks for a Wine app shortcut, or an agent needs
> to triage a Wine launch failure for a .NET/WinForms application.
